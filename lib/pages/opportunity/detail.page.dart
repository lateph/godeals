...
class DetailOpportunityState extends State<DetailOpportunityPage> {
  ...
  Widget _buildTab(dynamic data) {
    return new TabBarView(
      children: [
        ...
        _buildBidDialog(context, data),
        _buildDealDialog(context, data),
      ],
    );
  }
  ...
  Widget _buildDealDialog(BuildContext context, dynamic data) {
    List<dynamic> bids = data['deals'];
    return new Container(
      color: Colors.blueGrey[50],
      child: ListView.builder(
        itemCount: bids.length,
        itemBuilder: (BuildContext context, int index) {
          return _buildOnDeal(context, bids[index]);
        },
      ),
    );
  }
                  ...
                  new Container(
                    ...
                    child: new Row(
                      ...
                      children: <Widget>[
                        new Material(
                          child: Image.network(
                            appBloc.app.baseUrl + data['hotel']['images'][0]['thumbnailUrl'].toString(),
                            width: 80.0,
                            height: 80.0,
                          ),
                          borderRadius: BorderRadius.circular(5.0),
...
//                      _editEntry(context);
  ...
  Future<dynamic> loadDetail(BuildContext context, id) async {
    final AppBloc appBloc = AppBlocProvider.of(context);
...
//        data: fields,
    ...
    Response response3 = await appBloc.app.api.get(
      '/deal/list',
      data: {
        'opportunityId': id
      },
    ...
    return {
      ...
      'bids': response2.data['data'],
      'deals': response3.data['data']
    };
//      return {};
